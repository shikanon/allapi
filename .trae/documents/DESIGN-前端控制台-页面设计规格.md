...
* 空状态：展示“暂无数据/请先创建任务/请输入 task\_id”等文案与引导按钮
...
* 必填/选填字段按你 API 文档生成（占位）：
  ...
  * 输入视频源/参数（例如：source\_url、callback\_url、template 等——以你的接口为准）
...
1. 下方：调试日志（Recent Requests）

* 列表/表格：时间、动作（创建/查询/取消）、task\_id（若有）、结果（成功/失败）、耗时
